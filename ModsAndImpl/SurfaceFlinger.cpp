...
void SurfaceFlinger::handleMessageRefresh() {
    ...
    //zizhan: distiguish primary display and external display
    sp<DisplayDevice> displayDevicePrimary;
    sp<DisplayDevice> displayDeviceExternal;
    for (const auto& pair : mDisplays) {
        //const auto& displayDevice = pair.second;
        if(pair.second->isPrimary()) {
            displayDevicePrimary = pair.second; 
        }
        else {
            displayDeviceExternal = pair.second; 
        }
    }

    if(mDisplays.size() == 2){
        auto displayPrimary = displayDevicePrimary->getCompositionDisplay();
        auto displayExternal = displayDeviceExternal->getCompositionDisplay();

        beginFrame(displayDeviceExternal);
        prepareFrame(displayDeviceExternal);
        doDebugFlashRegions(displayDeviceExternal, repaintEverything);

        beginFrame(displayDevicePrimary);
        prepareFrame(displayDevicePrimary);
        doDebugFlashRegions(displayDevicePrimary, repaintEverything);

        
        if ( getHwComposer().hasClientComposition(displayPrimary->getId()) 
                && getHwComposer().hasClientComposition(displayExternal->getId()) )
        {
            //ALOGE("[zizhan] Both Primary and External is GPU composition, only compose once!!!");
            sp<GraphicBuffer> bufZizhan;
            base::unique_fd fdZizhan;
            base::unique_fd readyFenceZizhan;

            //ALOGE("[zizhan] dequeue begin------------------------------------------------------------");
            bufZizhan = displayExternal->getRenderSurface()->dequeueBuffer(&fdZizhan);
            fdZizhan.reset(-1);
            //zizhan: use the new compose
            doComposition(displayDevicePrimary, repaintEverything, bufZizhan, &fdZizhan, &readyFenceZizhan);
            
            //zizhan enqueue
            //sync_wait(fdZizhan.get(), -1);
            readyFenceZizhan.reset(-1);
            //ALOGE("[zizhan] enqueue begin------------------------------------------------------------");
            displayExternal->getRenderSurface()->queueBuffer(std::move(readyFenceZizhan));
            //ALOGE("[zizhan] enqueue end-------------------------------------------------------------");
            displayExternal->editState().dirtyRegion.clear();
            displayExternal->getRenderSurface()->flip();
        }
        else {
            //beginFrame(displayDevicePrimary);
            //prepareFrame(displayDevicePrimary);
            //doDebugFlashRegions(displayDevicePrimary, repaintEverything);
            doComposition(displayDevicePrimary, repaintEverything);

            //beginFrame(displayDeviceExternal);
            //prepareFrame(displayDeviceExternal);
            //doDebugFlashRegions(displayDeviceExternal, repaintEverything);
            doComposition(displayDeviceExternal, repaintEverything);
        }
    } else{
        beginFrame(displayDevicePrimary);
        prepareFrame(displayDevicePrimary);
...
void SurfaceFlinger::beginFrame(const sp<DisplayDevice>& displayDevice) {
    ...
    //ALOGE("[zizhan] BeginFrame for display [%s], must recompose is %d", displayDevice->getDebugName().c_str(), mustRecompose);
...
void SurfaceFlinger::doComposition(const sp<DisplayDevice>& displayDevice, bool repaintEverything, 
                                    sp<GraphicBuffer> bufZizhan, base::unique_fd* fdZizhan, base::unique_fd* readyFenceZizhan) {
    ATRACE_CALL();
    ALOGV("doComposition");
    ...
    if (displayState.isEnabled) {
        ...
        // repaint the framebuffer (if needed)
        doDisplayComposition(displayDevice, dirtyRegion, bufZizhan, fdZizhan, readyFenceZizhan);
...
void SurfaceFlinger::computeVisibleRegions(const sp<const DisplayDevice>& displayDevice,
    ...
    mDrawingState.traverseInReverseZOrder([&](Layer* layer) {
        ...
        //zizhan
        //ALOGE("[zizhan] display device: %s", displayDevice->getDisplayName().c_str());
        //ALOGE("[zizhan] display : %s", to_string(*display->getId()).c_str());
        //ALOGE("[zizhan] layer : %s", layer->getName().string());
        //ALOGE("[zizhan] layer stack: %d", layer->getLayerStack());
        //ALOGE("[zizhan] display stack: %d", displayDevice->getLayerStack());
        // only consider the layers on the given layer stack
        if (!display->belongsInOutput(layer->getLayerStack(), layer->getPrimaryDisplayOnly())) {
            return;
        }
...
void SurfaceFlinger::doDisplayComposition(const sp<DisplayDevice>& displayDevice,
    ...
    ALOGV("doDisplayComposition");
    base::unique_fd readyFence;
    if (!doComposeSurfaces(displayDevice, Region::INVALID_REGION, &readyFence, bufZizhan, fdZizhan)) return;
    readyFenceZizhan->reset(-1); 
    //readyFenceZizhan->reset(dup(readyFence));
    // swap buffers (presentation)
...
bool SurfaceFlinger::doComposeSurfaces(const sp<DisplayDevice>& displayDevice,
                                       ...
                                       const sp<GraphicBuffer> bufZizhan, 
                                       base::unique_fd* fdZizhan) {
    ATRACE_CALL();
    ...
    for (auto& layer : displayDevice->getVisibleLayersSortedByZ()) {
        ...
        if (!clip.isEmpty()) {
            //ALOGE("[zizhan] -------------------------------------------------------");
            //ALOGE("[zizhan] Display: %s", displayDevice->getDisplayName().c_str());
            //ALOGE("[zizhan] Layer: %s", layer->getName().string());
            //ALOGE("[zizhan] Composition type: %s", toString(layer->getCompositionType(displayDevice)).c_str());
            //ALOGE("[zizhan] -------------------------------------------------------");
    ...
    if (hasClientComposition) {
        ...
        //int fdZizhanInt = fdZizhan->get();
        //ALOGE("[zizhan] before wait, fdZizhan is %d", fdZizhanInt);
        //int err = sync_wait(fdZizhanInt, 3000);
        //if (err < 0) {
        //    ALOGE("[zizhan] fdZizhanInt sync time out");
        //    err = sync_wait(fdZizhanInt, -1);
        //}
        //close(fdZizhanInt);
        //fdZizhanInt = -1; 
        //ALOGE("[zizhan] after wait, fdZizhan is %d", fdZizhanInt);
        fdZizhan->reset(-1);

        renderEngine.drawLayers(clientCompositionDisplay, clientCompositionLayers,
                                buf->getNativeBuffer(), bufZizhan->getNativeBuffer(), /*useFramebufferCache=*/true, std::move(fd),
                                readyFence);
        //zizhan: copy the composited buffer. 
        //zizhan: should wait for the readFence
        /*
        ALOGE("[zizhan] copy begin ------------------------------------------------------");
        const Region viewportRegion(displayState.viewport);
        //uint8_t* src_bits = nullptr;
        //buf->lock(GRALLOC_USAGE_SW_READ_OFTEN, viewportRegion.bounds(),
        //    reinterpret_cast<void**>(&src_bits));
        //ALOGE("[zizhan] before wait, buffer is [%s], is it a nullptr?[%d], its len is [%u]", 
        //    src_bits, src_bits==nullptr, static_cast<uint32_t>(sizeof(src_bits)));
        //if (src_bits) buf->unlock();
        

        ALOGE("[zizhan] before wait, readyFence is %d", readyFence->get());
        if (!renderEngine.waitFence(std::move(*readyFence))) {
            ALOGE("failed to wait on fence fd");
            int err = sync_wait(readyFence->get(), 3000);
            if (err < 0) {
                ALOGE("[zizhan] gpu draw time out");
                err = sync_wait(readyFence->get(), -1);
            }
        }
        close(readyFence->get());
        readyFence->reset(-1);
        ALOGE("[zizhan] after wait, readyFence is %d", readyFence->get());
        
        int fdZizhanInt = fdZizhan->get();

        //ALOGE("[zizhan] before wait, fd is %d, fdZizhan is %d", fdInt, fdZizhanInt);
        //int err = sync_wait(fdInt, 3000);
        //if (err < 0) {
        //    ALOGE("[zizhan] fdInt sync time out");
        //    err = sync_wait(fdInt, -1);
        //}
        //close(fdInt);
        //fdInt = -1; 
        ...
        //buf->lock(GRALLOC_USAGE_SW_READ_OFTEN, viewportRegion.bounds(),
        //    reinterpret_cast<void**>(&src_bits));
        //ALOGE("[zizhan] after wait, buffer is [%s], is it a nullptr?[%d], its len is [%u]", 
        //    src_bits, src_bits==nullptr, static_cast<uint32_t>(sizeof(src_bits)));
        //if (src_bits) buf->unlock();
        

        //ALOGE("[zizhan] before copy, the fdzizhan is %d", fdZizhanInt);
        ALOGE("[zizhan] copy from buffer %p to buffer %p", buf->handle, bufZizhan->handle);

        uint8_t* dst_bits = nullptr;
        err = bufZizhan->lockAsync(GRALLOC_USAGE_SW_WRITE_OFTEN, viewportRegion.bounds(),
        reinterpret_cast<void**>(&dst_bits), fdZizhanInt);
        ALOGE_IF(err, "error locking dst buffer %s", strerror(-err));
        fdZizhanInt = -1;

        Region::const_iterator head(viewportRegion.begin());
        Region::const_iterator tail(viewportRegion.end());
        if (head != tail && dst_bits) {
            const Rect& r(*head);
            size_t width = r.getWidth();
            size_t height = r.getHeight();
            //std::vector<GLubyte> pixels(width * height * 4);
            checkErrors();
            glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, dst_bits);
            checkErrors();
            //uint8_t* s = &pixels[0];
            //uint8_t* d = dst_bits;
            //size_t size = width * height * 4;
            //ALOGE("[zizhan] before copy, s is [%s], d is [%s]", s, d);
            //memcpy(d, s, size);
            //ALOGE("[zizhan] after copy, dst_bits is [%s]", dst_bits);
        }
        if (dst_bits)
            bufZizhan->unlockAsync(&fdZizhanInt);
        //copyFrame(bufZizhan, buf, viewportRegion, &fdZizhanInt);
        //ALOGE("[zizhan] after copy, the fdzizhan is %d", fdZizhanInt);
        ALOGE("[zizhan] copy end ------------------------------------------------------");
        */
    } else if (displayId) {
...
void SurfaceFlinger::checkErrors() {
    do {
...
//zizhan -----------------------------------------------------------------------------------
status_t SurfaceFlinger::copyFrame(
        ...
        int *dstFenceFd)
{
    ...
    *dstFenceFd = -1;
    ...
    //zizhan
    //ALOGE("[zizhan] after lock, src_bits is [%p], dst_bits is [%p]", src_bits, dst_bits);
    //ALOGE("[zizhan] region is empty? [%d]", reg.isEmpty());
    ...
    if (head != tail && src_bits && dst_bits) {
        ...
        while (head != tail) {
            ...
            do {
                //ALOGE("[zizhan] before copy, s is [%s], d is [%s]", s, d);
                memcpy(d, s, size);
                //ALOGE("[zizhan] after copy, d is [%s]", d);
                //memset(d, 255, size); 
                d += dbpr;
                s += sbpr;
            } while (--h > 0);
...
bool SurfaceFlinger::doComposeSurfaces(const sp<DisplayDevice>& displayDevice,
    ...
    for (auto& layer : displayDevice->getVisibleLayersSortedByZ()) {
        ...
        ALOGV("Layer: %s", layer->getName().string());
        ALOGV("  Composition type: %s", toString(layer->getCompositionType(displayDevice)).c_str());
        //zizhan: only show these two logs
        //ALOGE("[zizhan] -------------------------------------------------------");
        //ALOGE("[zizhan] Display: %s", displayDevice->getDisplayName().c_str());
        //ALOGE("[zizhan] Layer: %s", layer->getName().string());
        //ALOGE("[zizhan] Composition type: %s", toString(layer->getCompositionType(displayDevice)).c_str());
