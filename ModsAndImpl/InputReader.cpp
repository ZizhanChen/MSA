...
std::optional<DisplayViewport> TouchInputMapper::findViewport() {
    //ALOGE("[zizhan] always return the internal viewport");
    std::optional<DisplayViewport> viewport = mConfig.getDisplayViewportByType(ViewportType::VIEWPORT_INTERNAL);
    return viewport;
    if (mParameters.hasAssociatedDisplay) {
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    //ALOGE("eventhub displayid = %d, mViewport.orientation=%d, mViewport.physical=[%d, %d, %d, %d], mViewport.logical=[%d, %d, %d, %d]", 
    //        mViewport.displayId, mViewport.orientation, 
    //        mViewport.physicalLeft, mViewport.physicalTop, mViewport.physicalRight, mViewport.physicalBottom,
    //        mViewport.logicalLeft, mViewport.logicalTop, mViewport.logicalRight, mViewport.logicalBottom);
...
void TouchInputMapper::cookPointerData() {
    ...
    for (uint32_t i = 0; i < currentPointerCount; i++) {
        ...
        //ALOGE("eventhub x = %f, y = %f, left=%f, right=%f, bottom = %f, top = %f" , x, y, left, right, bottom, top);
        ...
        if (strcmp(mCombineMode, "on") == 0){
            if (strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.c_str(), "usb-fe3a0000.usb-1/input0") == 0){
                if (mSurfaceOrientation == DISPLAY_ORIENTATION_0){
                    y = y / 2;
                }
                else if (mSurfaceOrientation == DISPLAY_ORIENTATION_180){
                    y = (y + mViewport.deviceHeight) / 2;
                } else if (mSurfaceOrientation == DISPLAY_ORIENTATION_90) {
                    x = x / 2; 
                } else if (mSurfaceOrientation == DISPLAY_ORIENTATION_270) {
                    x = (x + mViewport.deviceWidth) / 2;
                }
            }
            else if (strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.c_str(), "usb-xhci-hcd.0.auto-1/input0") == 0){
                if (mSurfaceOrientation == DISPLAY_ORIENTATION_0){
                    y = (y + mViewport.deviceHeight) / 2;
                } else if (mSurfaceOrientation == DISPLAY_ORIENTATION_180){
                    y = y / 2;
